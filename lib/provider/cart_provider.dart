...
import 'package:maidc_seller/view/screens/order/invoice_screen.dart';
...
class CartProvider extends ChangeNotifier {
  ...
  void setQuantity(BuildContext context, bool isIncrement, int? index,
    ...
    if (isIncrement) {
      ...
      if (_customerCartList[_customerIndex].cart![index!].quantity! > 1) {
        showCustomSnackBar(getTranslated('quantity_updated', context), context,
            isToaster: true, isError: false);
        _customerCartList[_customerIndex].cart![index].quantity =
            _customerCartList[_customerIndex].cart![index].quantity! - 1;
        for (int i = 0;
  ...
  int? get selectedCustomerIdForReviewFilter =>
      _selectedCustomerIdForReviewFilter;
  ...
  Future<void> getCustomerList(String type) async {
    ...
    _isGetting = true;
    ApiResponse response = await cartRepo!.getCustomerList(type);
    if (response.response!.statusCode == 200) {
      _searchedCustomerList = [];
      _searchedCustomerList!
