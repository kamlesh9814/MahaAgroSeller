...
import 'package:maidc_seller/data/model/response/brand_model.dart';
import 'package:maidc_seller/data/model/response/category_model.dart';
import 'package:maidc_seller/data/model/response/config_model.dart';
...
class SellerProvider extends ChangeNotifier {
  ...
  getTitleAndDescriptionList(
      List<Language> languageList, EditProduct? edtProduct) {
  ...
  Future addProductImage(BuildContext context, ImageModel imageForUpload,
      Function callback) async {
    productReturnImage = [];
    _isLoading = true;
    notifyListeners();
    ApiResponse response = await shopRepo!
        .addImage(context, imageForUpload, attributeList![0].active);
    if (response.response != null && response.response!.statusCode == 200) {
      _isLoading = false;
  ...
  Future<void> addProduct(
    ...
    ApiResponse response = await shopRepo!.addProduct(
        product,
        addProduct,
        fields,
        productImages,
  ...
  Future<ApiResponse> uploadDigitalProduct(String token) async {
    ...
    ApiResponse response =
        await shopRepo!.uploadDigitalProduct(_selectedFileForImport, token);
  ...
  Future<ApiResponse> uploadReadyAfterSellDigitalProduct(BuildContext context,
    ...
    _isLoading = true;
    notifyListeners();
    ApiResponse response = await shopRepo!.uploadAfterSellDigitalProduct(
  ...
  Future<void> updateProductQuantity(BuildContext context, int? productId,
    ...
    _isLoading = true;
    ApiResponse apiResponse = await shopRepo!
        .updateProductQuantity(productId, currentStock, updatedVariations);
