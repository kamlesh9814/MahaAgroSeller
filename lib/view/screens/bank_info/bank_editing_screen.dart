...
class BankEditingScreenState extends State<BankEditingScreen> {
  ...
  TextEditingController? _icfcController;
  ...
  final FocusNode _icfcNode = FocusNode();
  ...
  _updateUserAccount() async {
    ...
    String icfc = _icfcController!.text.trim();
...

    {
    ...
    else if (icfc.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
          content: Text(('enter_ifsc_code')),
          backgroundColor: ColorResources.red));
    }
      ...
      updateUserInfoModel.ifsc = _icfcController?.text ?? '';
      ...
      SellerBody sellerBody = SellerBody(
        ...
        ifsc: _icfcController?.text ?? '',
      );

      await Provider.of<BankInfoProvider>(context, listen: false).updateUserInfo(context, updateUserInfoModel, sellerBody,
        ...
        if (response!.isSuccess) {
          ...
          showCustomSnackBar(
              getTranslated('bank_info_updated_successfully', context), context,
              isError: false);
        } else {
  ...
  void initState() {
    ...
    _icfcController = TextEditingController();
    ...
    _icfcController!.text = widget.sellerModel!.ifsc ?? '';
  }
