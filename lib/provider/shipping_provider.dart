...
import 'package:maidc_seller/data/model/response/base/error_response.dart';
import 'package:maidc_seller/data/model/response/category_wise_shipping_model.dart';
import 'package:maidc_seller/data/model/response/shipping_model.dart';
...
class ShippingProvider extends ChangeNotifier {
  ...
  Future updateShippingMethod(String? title, String? duration, double? cost,
    ...
    ApiResponse apiResponse =
        await shippingRepo!.updateShipping(title, duration, cost, id);
  ...
  Future<void> deleteShipping(int? id) async {
    ...
    if (response.response!.statusCode == 200) {
      ...
      showCustomSnackBar(
          ...
          Get.context!,
          isError: false);
      getShippingList(Provider.of<AuthProvider>(Get.context!, listen: false)
          .getUserToken());
    } else {
  ...
  Future<void> getCategoryWiseShippingMethod() async {
    ApiResponse apiResponse =
        await shippingRepo!.getCategoryWiseShippingMethod();
    if (apiResponse.response != null &&
        apiResponse.response!.statusCode == 200) {
      _categoryWiseShipping = [];
      _isMultiply = [];
  ...
  Future<void> getSelectedShippingMethodType(BuildContext context) async {
    ApiResponse apiResponse =
        await shippingRepo!.getSelectedShippingMethodType();
    if (apiResponse.response != null &&
        apiResponse.response!.statusCode == 200) {
  ...
  Future<ApiResponse> setCategoryWiseShippingCost(BuildContext context,
    ...
    notifyListeners();
    ApiResponse apiResponse =
        await shippingRepo!.setCategoryWiseShippingCost(ids, cost, multiPly);
    if (apiResponse.response != null &&
        apiResponse.response!.statusCode == 200) {
  ...
  void setShippingTypeIndex(BuildContext context, int value,
    ...
    if (_selectedShippingTypeIndex == 0) {
      ...
      Navigator.pushReplacement(context,
          MaterialPageRoute(builder: (_) => const ProductWiseShipping()));
    }
    notifyListeners();
  }
